...
// GenerateInvalidRowsFile creates a CSV with only invalid/damaged rows
func (p *CSVProcessor) GenerateInvalidRowsFile(outputDir string) (string, error) {
	if len(p.InvalidRows) == 0 {
		return "", nil // No invalid rows to export
	}

	timestamp := time.Now().Format("20060102_150405")
	outputPath := filepath.Join(outputDir, fmt.Sprintf("damaged_rows_%s.csv", timestamp))

	outFile, err := os.Create(outputPath)
	if err != nil {
		return "", fmt.Errorf("failed to create damaged rows file: %w", err)
	}
	defer outFile.Close()
	...
	// Write headers
	headers := []string{"id", "date", "description", "amount", "category", "error_message"}
	if err := writer.Write(headers); err != nil {
		return "", fmt.Errorf("failed to write headers: %w", err)
	}

	// Write only invalid rows with error messages
	for _, row := range p.InvalidRows {
		record := []string{
			strconv.Itoa(row.ID),
			"", // Date may be invalid
